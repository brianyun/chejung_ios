...
class SlideInLauncher: NSObject, UITextFieldDelegate {
    ...
    func showFinish() {
        ...
        if let window = UIApplication.shared.keyWindow {
            ...
            successLabel.font = UIFont(name: "Futura-Medium", size: 50.0)
            
            tapLabel.textColor = UIColor.white
            tapLabel.text = "tap to dismiss"
            tapLabel.textAlignment = NSTextAlignment.center
            tapLabel.font = UIFont(name: "Futura-Medium", size: 25.0)
            
            finishView.frame = CGRect(x: 0, y: 0, width: window.frame.width, height: 0)
            successLabel.frame = finishView.frame
            
            tapLabel.sizeToFit()
            tapLabel.center.x = successLabel.center.x
            tapLabel.center.y = successLabel.center.y + 100
            ...
            UIView.animate(withDuration: 0.5, animations: {
                ...
                self.successLabel.frame = self.finishView.frame
                self.tapLabel.center.y = self.successLabel.center.y + 100
            })
        }
